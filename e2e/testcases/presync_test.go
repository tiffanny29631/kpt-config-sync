...
func TestAddPreSyncAnnotationRepoSync(t *testing.T) {
	...
	err = nt.Watcher.WatchObject(kinds.RepoSyncV1Beta1(), repoSyncID.Name, repoSyncID.Namespace,
		[]testpredicates.Predicate{
			testpredicates.MissingAnnotation(metadata.ImageURLAnnotationKey),
			testpredicates.MissingAnnotation(metadata.SourceCommitAnnotationKey),
		})
	if err != nil {
...
func checkRootSyncPreSyncAnnotations() testpredicates.Predicate {
	return func(o client.Object) error {
		...
		repoAnnotation, ok := o.GetAnnotations()[metadata.ImageURLAnnotationKey]
		if !ok {
			return fmt.Errorf("object %q does not have annotation %q", o.GetName(), metadata.ImageURLAnnotationKey)
		}
